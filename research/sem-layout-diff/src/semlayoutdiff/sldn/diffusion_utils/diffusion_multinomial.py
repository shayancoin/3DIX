...
class MultinomialDiffusion(torch.nn.Module):
    """
    Multinomial diffusion model for discrete data generation.
    
    Implements multinomial diffusion process with support for conditional generation
    through floor plans, room types, text, and mixed conditions.
    """
    ...
    def p_pred(self, log_x, t, floor_plan, room_type, text_condition, mixed_condition_id):
        if self.parametrization == 'x0':
            log_x_recon = self.predict_start(log_x, t=t, floor_plan=floor_plan, room_type=room_type, text_condition=text_condition, mixed_condition_id=mixed_condition_id)
            log_model_pred = self.q_posterior(
                log_x_start=log_x_recon, log_x_t=log_x, t=t)
        elif self.parametrization == 'direct':
            log_model_pred = self.predict_start(log_x, t=t, floor_plan=floor_plan, room_type=room_type, text_condition=text_condition, mixed_condition_id=mixed_condition_id)
        else:
            raise ValueError
    ...
    @torch.no_grad()
    def p_sample(self, log_x, t, floor_plan, room_type, text_condition, mixed_condition_id):
        model_log_prob = self.p_pred(log_x=log_x, t=t, floor_plan=floor_plan, room_type=room_type, text_condition=text_condition, mixed_condition_id=mixed_condition_id)
    ...
    def compute_Lt(self, log_x_start, log_x_t, t, floor_plan, room_type, text_condition, mixed_condition_id, detach_mean=False):
        ...
        log_model_prob = self.p_pred(log_x=log_x_t, t=t, floor_plan=floor_plan, room_type=room_type, text_condition=text_condition, mixed_condition_id=mixed_condition_id)
    ...
    def _train_loss(self, x, floor_plan, room_type, text_condition, mixed_condition_id):
    ...
    def log_prob(self, x, floor_plan=None, room_type=None, text_condition=None, mixed_condition_id=None):
        b, device = x.size(0), x.device
        if self.training:
            return self._train_loss(x, floor_plan, room_type, text_condition, mixed_condition_id)
    ...
    def sample_chain(self, num_samples, floor_plan=None, room_type=None, text_condition=None, mixed_condition_id=None):
        b = num_samples
        device = self.log_alpha.device
