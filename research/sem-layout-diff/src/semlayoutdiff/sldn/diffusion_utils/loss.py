...
def loglik_bpd(model, x, floor_plan, room_type, text_condition, mixed_condition_id):
    """Compute the log-likelihood in bits per dim."""
    return - model.log_prob(x, floor_plan=floor_plan, room_type=room_type, text_condition=text_condition, mixed_condition_id=mixed_condition_id).sum() / (math.log(2) * x.shape.numel())
...
def elbo_bpd(model, x, floor_plan, room_type, text_condition=None, mixed_condition_id=None):
    """
    Compute the ELBO in bits per dim.
    Same as .loglik_bpd(), but may improve readability.
    """
    return loglik_bpd(model, x, floor_plan, room_type, text_condition, mixed_condition_id)
